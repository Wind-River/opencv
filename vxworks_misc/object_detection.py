...
parser.add_argument('--asyn', type=int, default=0,
                    help='Number of asynhronous forwards at the same time. '
...
cap = cv.VideoCapture(cv.samples.findFileOrKeep(args.input) if args.input else 4)
...
def processingThreadBody():
    ...
    while process:
        ...
        try:
            ...
            if args.asyn:
                if len(futureOutputs) == args.asyn:
        ...
        if not frame is None:
            ...
            if args.asyn:
                futureOutputs.append(net.forwardasyn())
            else:
...
while cv.waitKey(1) < 0:
    try:
        ...
        cv.imwrite("detection.png", frame)
    except queue.Empty:
        pass
